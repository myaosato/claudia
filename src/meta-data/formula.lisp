...
;; rewrite
(defmethod rewrite ((x ∧) reduction)
  (∧ (rewrite (∧-0 x) reduction) (rewrite (∧-1 x) reduction)))
(defmethod rewrite ((x ∨) reduction)
  (∨ (rewrite (∨-0 x) reduction) (rewrite (∨-1 x) reduction)))
(defmethod rewrite ((x ¬) reduction)
  (¬ (rewrite (¬-0 x) reduction)))
(defmethod rewrite ((x →) reduction)
  (→ (rewrite (→-0 x) reduction) (rewrite (→-1 x) reduction)))
(defmethod rewrite ((x ∀) reduction)
  (∀ (∀-var x) (rewrite (∀-formula x) reduction)))
(defmethod rewrite ((x ∃) reduction)
  (∃ (∃-var x) (rewrite (∃-formula x) reduction)))
(defmethod rewrite ((x predicate) reduction)
  (apply #'predicate (mapcar (lambda (x) (rewrite x reduction)) (predicate-terms x))))
(defmethod rewrite ((x prop) reduction)
  x)
